...
async def process_data(data: Union[str, List[Dict[str, str]], Dict[str, str]], queues, futures_dict, logger):
    try:
        ...
        logger.info(f'Received data: {data}. Assigned correlation_id: {correlation_id}.')
        ...
        for queue in queues:
            ...
            logger.info(f'Pushed to {id(queue)} : {message}')
        ...
        result = await future
        logger.info(f'Received result for correlation_id: {correlation_id}. Result: {result}')
        return result
